...
def add_to_cart(request):
    if request.user.is_authenticated and not request.user.is_anonymous and 'pro_id' in request.GET and 'qty' in request.GET and 'price' in request.GET:
...
def remove_from_cart(requset, orderdetails_id):
    if requset.user.is_authenticated and not requset.user.is_anonymous and orderdetails_id:
        orderdetails = OrderDetails.objects.get(id=orderdetails_id)
        if orderdetails.order.user.id==requset.user.id:
            orderdetails.delete()
    return redirect('cart')


def add_qty(requset, orderdetails_id):
    if requset.user.is_authenticated and not requset.user.is_anonymous and orderdetails_id:
        orderdetails = OrderDetails.objects.get(id=orderdetails_id)
        if orderdetails.order.user.id==requset.user.id:
...
def sub_qty(requset, orderdetails_id):
    if requset.user.is_authenticated and not requset.user.is_anonymous and orderdetails_id:
        orderdetails = OrderDetails.objects.get(id=orderdetails_id)
        if orderdetails.order.user.id==requset.user.id:
